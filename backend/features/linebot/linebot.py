...
from backend.features.quiz.quiz_handler import FullInsuranceQuizHandler
from backend.core.database import SessionLocal
from backend.features.risk.bigru_service import predict_risk_for_user
from backend.features.risk.notification_service import check_and_notify
from backend.features.risk.risk_prediction import RiskPrediction
from backend.features.web.user import User
from backend.features.linebot.conversation_memory import ConversationMemory
from backend.features.wishlist.wishlist_model import Wishlist
from backend.features.expense.record import Record
from backend.features.news.daily_news_service import run_daily_news_pipeline
from backend.features.tax.tax_calculator import calculate_taiwan_tax_2026
from backend.core.token_tracker import upsert_pipeline_tokens, is_over_daily_limit
from backend.core.response_logger import log_response
...
def _run_ml_risk_push(user_id: int, line_user_id: str) -> None:
    ...
    try:
        with SessionLocal() as _db:
            ...
            try:
                from backend.features.financial_status.financial_status_service import compute_and_store
...
def handle_message(event):
    ...
    if intent == "credit_card":
        ...
        from backend.features.qa_rag.rag_service import answer_financial_question
