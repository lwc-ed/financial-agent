...
# 載入三個 Ensemble 模型
print(f"📦 載入 V{VERSION} Ensemble 模型（{len(ENSEMBLE_SEEDS)} seeds）...")
models = []
for seed in ENSEMBLE_SEEDS:
    ckpt  = torch.load(f"{ARTIFACTS_DIR}/finetune_gru_v5_seed{seed}.pth", map_location=device)
    hp    = ckpt["hyperparams"]
    ...
    m.eval()
    models.append(m)
    print(f"  ✅ Seed {seed} 載入完成（best val_loss={ckpt['val_loss']:.6f}）")

# 載入 Log1p scaler（V5 使用 Log1p）
with open(f"{ARTIFACTS_DIR}/log_target_scaler_v5.pkl", "rb") as f:
...
def ensemble_predict_logspace(X):
    """
    三個模型在 log1p 標準化空間的平均預測
    然後反轉換到實際金額
    """
...
y_val_pred_raw  = ensemble_predict_logspace(X_val)
y_val_true      = get_true_real(y_val)
y_test_pred_raw = ensemble_predict_logspace(X_test)
y_test_true     = get_true_real(y_test)


# ─────────────────────────────────────────
# Per-user 偏差修正
# 理由：用驗證集計算每個 user 的系統性偏差（中位誤差），
#       在測試集預測中修正，不洩漏測試集資訊。
# ─────────────────────────────────────────
print("\n📐 Per-user 偏差修正...")
user_bias = {}
...
# 套用偏差修正到測試集
y_test_pred_corrected = y_test_pred_raw.copy()
for u in np.unique(test_user_ids):
    mask = np.array(test_user_ids) == u
    bias = user_bias.get(str(u), 0.0)
    y_test_pred_corrected[mask] -= bias

# 同樣修正驗證集（用於評估修正效果）
y_val_pred_corrected = y_val_pred_raw.copy()
for u in np.unique(val_user_ids):
    mask = np.array(val_user_ids) == u
    bias = user_bias.get(str(u), 0.0)
    y_val_pred_corrected[mask] -= bias

# 確保非負
y_test_pred_corrected = np.clip(y_test_pred_corrected, 0, None)
y_val_pred_corrected  = np.clip(y_val_pred_corrected,  0, None)
...
test_nmae  = per_user_nmae(y_test_true, y_test_pred_corrected, test_user_ids)

# 不修正的指標（對比）
val_mae_raw   = float(np.mean(np.abs(y_val_pred_raw  - y_val_true)))
test_mae_raw  = float(np.mean(np.abs(y_test_pred_raw - y_test_true)))
print(f"\n  偏差修正效果：Val MAE {val_mae_raw:,.2f} → {val_mae:,.2f}（{'改善' if val_mae < val_mae_raw else '退步'}）")
print(f"  偏差修正效果：Test MAE {test_mae_raw:,.2f} → {test_mae:,.2f}（{'改善' if test_mae < test_mae_raw else '退步'}）")
...
all_versions = [
    ...
    ("v3", f"{ARTIFACTS_DIR}/metrics_v3.json"),
    ("v4", f"{ARTIFACTS_DIR}/metrics_v4.json"),
]
for name, path in all_versions:
...
print(f"  {'v5(best)':12s}: Test MAE={test_mae:>8,.2f}  SMAPE={test_smape:>6.2f}%  NMAE={test_nmae:>7.2f}%")
