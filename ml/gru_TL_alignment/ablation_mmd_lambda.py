...
# ── 載入資料（只做一次）───────────────────────────────────────────────────────
print("\n📂 載入資料...")
X_wmt_train = np.load(f"{ARTIFACTS_DIR}/walmart_aligned_X_train.npy")
y_wmt_train = np.load(f"{ARTIFACTS_DIR}/walmart_aligned_y_train.npy")
X_wmt_val   = np.load(f"{ARTIFACTS_DIR}/walmart_aligned_X_val.npy")
y_wmt_val   = np.load(f"{ARTIFACTS_DIR}/walmart_aligned_y_val.npy")

X_per_train = np.load(f"{ARTIFACTS_DIR}/personal_aligned_X_train.npy")
y_per_train = np.load(f"{ARTIFACTS_DIR}/personal_aligned_y_train.npy")
X_per_val   = np.load(f"{ARTIFACTS_DIR}/personal_aligned_X_val.npy")
y_per_val   = np.load(f"{ARTIFACTS_DIR}/personal_aligned_y_val.npy")
X_per_test  = np.load(f"{ARTIFACTS_DIR}/personal_aligned_X_test.npy")

y_per_val_raw  = np.load(f"{ARTIFACTS_DIR}/personal_aligned_y_test_raw.npy")
y_per_test_raw = np.load(f"{ARTIFACTS_DIR}/personal_aligned_y_test_raw.npy")
...
y_per_val_scaled = np.load(f"{ARTIFACTS_DIR}/personal_aligned_y_val.npy")
y_per_val_raw    = target_scaler.inverse_transform(y_per_val_scaled)
...
def run_pretrain(mmd_lambda: float, save_dir: str) -> float:
    """回傳最佳 val_loss"""
    model      = GRUWithAttention().to(device)
...
# ── 比較表 ────────────────────────────────────────────────────────────────────
print(f"\n\n{'='*72}")
print(f"  MMD λ Ablation Study 結果（Test，越低越好）")
print(f"{'='*72}")
...
# ── 儲存 ──────────────────────────────────────────────────────────────────────
results_serializable = {str(k): v for k, v in all_results.items()}
