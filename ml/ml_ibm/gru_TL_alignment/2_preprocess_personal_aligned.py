"""
Step 2：個人資料 Aligned 前處理
================================
使用與 IBM 完全相同的 Rolling Z-score pipeline
輸出：
  - personal_aligned_X_train/val/test.npy
  - personal_aligned_y_train/val/test.npy
  - personal_aligned_target_scaler.pkl
  - personal_aligned_train/val/test_user_ids.npy
  - personal_aligned_y_train_risk_labels.npy  ← 新增：4-class risk label（MT finetune 用）
  - personal_aligned_y_val_risk_labels.npy    ← 新增
"""
...
np.save(f"{ARTIFACTS_DIR}/personal_aligned_y_train_risk_labels.npy", train_risk)
np.save(f"{ARTIFACTS_DIR}/personal_aligned_y_val_risk_labels.npy",   val_risk)
...
print("  ✅ personal_aligned_y_test_raw.npy")
print("  ✅ personal_aligned_y_train_risk_labels.npy  ← 新增 MT label")
print("  ✅ personal_aligned_y_val_risk_labels.npy    ← 新增 MT label")
print(f"\n🎉 完成！下一步：執行 3_pretrain_aligned.py")
