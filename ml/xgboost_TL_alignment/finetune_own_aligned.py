...
def main():
    ...
    metrics = {
        "mae": float(mean_absolute_error(y_valid_raw, preds_raw)),
        "rmse": float(mean_squared_error(y_valid_raw, preds_raw) ** 0.5),
    }
    ...
    print(f"[OK] saved valid predictions -> {RESULT_DIR / 'own_valid_predictions_finetune.csv'}")
    print(metrics)
