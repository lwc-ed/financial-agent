"""
Step 6：三級警報評估（kyh v2 aligned XGBoost）
==============================================
將回歸預測轉為三個等級：
  🟢 正常    ：預測值 < 個人基線 × 1.2
...
輸出：results/tiered_alert_evaluation_v2.json
"""
...
def load_or_generate_predictions(test_df: pd.DataFrame, feature_cols: list[str]) -> pd.DataFrame:
    ...
    model_path = MODEL_DIR / "xgb_finetuned_own_aligned_v2.json"
    ...
    X_sc = scaler.transform(test_df[feature_cols].copy())
...
def main():
    ...
    print(f"\n{'=' * 65}")
    print(f"  三級警報評估（kyh v2 aligned XGBoost，thresholds: {LOW_RATIO}× / {HIGH_RATIO}×）")
    print(f"{'=' * 65}")
    print("\n  混淆矩陣（行=真實，列=預測）：")
    ...
    output = {
        "model": "kyh v2 aligned XGBoost (finetuned)",
        "low_ratio": LOW_RATIO,
    ...
    output_path = RESULT_DIR / "tiered_alert_evaluation_v2.json"
