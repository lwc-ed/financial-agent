...
本次實驗目標是迭代 `ml/ml_ibm/bigru_TL_alignment`，使 IBM TL 在排除 `user14` 後，相較於以下兩組模型都能在四個指標上顯著提升：

- `ml/ml_walmart/bigru_TL_alignment`
- `ml/ml_walmart/bigru`
...
- `ml/ml_ibm/bigru_TL_alignment/4_finetune_bigru.py`
- `ml/ml_ibm/bigru_TL_alignment/5_predict_bigru.py`
...
|---:|---|---|---:|---:|---:|---:|---|
| 1 | 以既有兩階段 finetune 加乘法 validation calibration 重跑，作為本次迭代基準。 | `per_seed_metrics_20260524_030556.csv` | 778.655952 | 1163.275607 | 0.872925 | 0.773559 | 未通過同名 `ml/ml_walmart/bigru_TL_alignment` 比較：`Binary_F1` 顯著變差；Wilcoxon 的 RMSE 在 alpha=0.01 不足。相較 `ml/ml_walmart/bigru` baseline 已通過。 |
| 2 | 新增 validation affine calibration：scale 0.82-1.10、offset -250 到 250，並提高 calibration score 中 `Binary_F1` 權重。 | `per_seed_metrics_20260524_033920.csv` | 780.612741 | 1149.733341 | 0.875907 | 0.779943 | 未通過同名比較：`Binary_F1` 仍顯著變差。相較 `ml/ml_walmart/bigru` baseline 仍通過。 |
| 3 | 新增 risk-ratio-aware boundary calibration，並將 Phase 2 縮短為 25 epochs、patience 8。 | `per_seed_metrics_20260524_040328.csv` | 785.949591 | 1147.741175 | 0.874806 | 0.781836 | 未通過同名比較：`Binary_F1` 仍顯著變差。相較 `ml/ml_walmart/bigru` baseline 仍通過。 |
...
### IBM TL vs `ml/ml_walmart/bigru_TL_alignment`，排除 user14

來源輸出：

- `ml/ml_ibm/statistical_test/output/pair_t_test_results.txt`
- `ml/ml_ibm/statistical_test/output/wilcoxon_results.txt`
...
### IBM TL vs `ml/ml_walmart/bigru`，排除 user14

來源輸出：

- `ml/ml_ibm/statistical_test/output/pair_t_test_baseline_results.txt`
- `ml/ml_ibm/statistical_test/output/wilcoxon_baseline_results.txt`
...
主要失敗原因一開始不是訓練本身，而是 calibration。第 1 到第 3 輪的 validation-selected calibration 能改善 MAE、RMSE 或 Weighted_F1，但會把預測值往降低 alarm 的方向推，導致相較較強的 `ml/ml_walmart/bigru_TL_alignment_exclude_user14` 時，`Binary_F1` 顯著變差。
...
遠端環境的 `ml/ml_walmart/bigru_TL_alignment_exclude_user14` baseline 較強，使第 4 輪的 `Binary_F1` margin 不夠穩定。第 5 輪改用 validation-based checkpoint selection，讓 checkpoint 選擇直接反映排除 `user14` 後的分類任務表現；最終 `Binary_F1` 平均值提升到 0.892224，並在 paired t-test 與 Wilcoxon 中都達到 alpha=0.01 顯著。
...
### Full-user 最終結果：IBM TL vs `ml/ml_walmart/bigru_TL_alignment`
...
### Full-user 最終結果：IBM TL vs `ml/ml_walmart/bigru`
...
```bash
rm -f /Users/liweichen/financial-agent/ml/ml_ibm/bigru_TL_alignment/artifacts_bigru_tl/finetune_bigru_seed*.pth
cd /Users/liweichen/financial-agent/ml/ml_ibm/bigru_TL_alignment
../../ml/bigru/venv/bin/python 4_finetune_bigru.py
../../ml/bigru/venv/bin/python 5_predict_bigru.py
ls -lt /Users/liweichen/financial-agent/ml/ml_ibm/model_outputs/bigru_TL_alignment_exclude_user14/per_seed_metrics_*.csv | head
cd /Users/liweichen/financial-agent/ml/ml_ibm/statistical_test
/Users/liweichen/financial-agent/ml/ml_walmart/bigru/venv/bin/python pair_t_test.py
/Users/liweichen/financial-agent/ml/ml_walmart/bigru/venv/bin/python pair_t_test_baseline.py
/Users/liweichen/financial-agent/ml/ml_walmart/bigru/venv/bin/python wilcoxon_test.py
/Users/liweichen/financial-agent/ml/ml_walmart/bigru/venv/bin/python wilcoxon_test_baseline.py
```
