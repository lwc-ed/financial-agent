...
def main():
    ...
    txt_path = OUT_DIR / "pair_t_test_baseline_results.txt"
    with open(txt_path, "w") as f:
        f.write("\n\n".join(all_tables) + "\n")

    json_path = OUT_DIR / "pair_t_test_baseline_results.json"
    with open(json_path, "w") as f:
        json.dump(all_json, f, ensure_ascii=False, indent=2)

    md_path = OUT_DIR / "pair_t_test_baseline_results.md"
    with open(md_path, "w") as f:
        f.write("# Paired T-Test Results (no-TL baseline vs ml_ibm TL)\n\n")
